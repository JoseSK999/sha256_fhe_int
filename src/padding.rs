// This module contains the padding function, which is computed by the client over the plain text. The function
// returns the padded data as a vector of bools, for later encryption. Note that padding could also be performed
// by the server, by appending trivially encrypted bools. However, in our implementation, the exact length of
// the pre-image (hashed message) is not revealed.

// If input starts with "0x" and following characters are valid hexadecimal values, it's interpreted as hex,
// otherwise input is interpreted as text
pub fn pad_sha256_input(input: &str) -> Vec<u32> {
...
fn pad_sha256_data(data: &[u8]) -> Vec<u32> {
    ...
    bools_to_u32s_big_endian(bits)
}

fn bools_to_u32s_big_endian(bools: Vec<bool>) -> Vec<u32> {
    // Create a new vector to store the resulting u32 values
    let mut result = Vec::with_capacity(bools.len() / 32);

    // Iterate over the padded bool vector, 32 elements at a time
    for chunk in bools.chunks(32) {
        let mut value = 0;

        // Convert each chunk of 32 bools into a u32 value
        ...
        // Push the u32 value into the resulting vector
        result.push(value);
    }
...
pub fn u32s_to_hex_string(u32s: Vec<u32>) -> String {
    let mut hex_string = String::new();

    for value in u32s {
        // Format each u32 as an 8-character hexadecimal string with leading zeros
...
mod tests {
    ...
    fn test_pad_sha256_input() {
        ...
        let hex_result = u32s_to_hex_string(result);
